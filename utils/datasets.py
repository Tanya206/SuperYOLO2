...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.stride = stride
        if hr_input:
            self.img_path = "dataset/NWPU_VHR10_1024/images/"
        else:
            self.img_path = "dataset/NWPU_VHR10_512/images/"



        with open(path, "r") as file:
            self.img_files = [x.strip() for x in file.readlines()]
        for j in range(len(self.img_files)):
            self.img_files[j] = self.img_path + self.img_files[j]
 #+ '_co.png' #self.img_files[j].rstrip() + '_co.png'  #self.img_path + self.img_files[j].rstrip() + '_co.png'
